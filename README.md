...
文字盤に **何行出すか（2〜4）** と、**そのうち何行を予定に使うか（0〜行数）** を iPhone 画面上部で選ぶ。

| 例 | 行 | 予定 | 文字盤 |
|---|---|---|---|
| リマインダーだけ | 2 | 0 | リマインダー2件 |
| 1つずつ | 2 | 1 | リマインダー1件＋次の予定 |
| 予定だけ | 3 | 3 | 直近の予定3件 |
| 盛り | 4 | 2 | リマインダー2件＋予定2件 |

並びはリマインダー → 予定。片方が足りなければもう片方で埋める。行数が増えるほど文字は小さくなる（長い方に合わせて全行同じ倍率）。

予定は「**これから24時間**」のもの。翌日の分は「明日09:00 会議」のように出る。
時刻のある予定は、始まってから1時間までは残る。終日の予定は、行が余れば後ろに「終日 ◯◯」と出る
（予定の枠の下の「**終日の予定**」で出さないようにもできる）。
**祝日は出さない。** 名前に「祝日」「休日」「祭日」「Holiday」を含むカレンダーは読まない
（iOS に祝日かどうかを返す API は無い）。名前と開始が同じ予定は、別のカレンダーから来ても1つにまとめる。
時刻が来た予定は **Watch のウィジェットが自分で落とす**（開始時刻ごとにタイムラインを刻んでいる）ので、iPhone からの再送は要らない。

Google カレンダーは iOS の 設定 → カレンダー → アカウント に Google を入れておけば、EventKit 経由で同じように読める。
API キーや OAuth は不要。

## iPhone のホーム画面ウィジェット（v6）

純正リマインダーのウィジェットと同じ作り。横長（4アプリ分）と縦長に対応。
○を押すと打ち消し線＋3秒のカウントダウン、その間にもう一度押せば取り消し。設定は本体アプリで行う。

ホーム画面を長押し → ＋ → 「盤面タスク」で追加。

## 色

リマインダーと予定の色は、iPhone 画面上部の＋−の左にある丸をタップするたびに15色を順ぐり。
文字盤・Watch アプリ・iPhone・ホームウィジェットの全部に効く。

## 投げ銭（コーヒーを奢る）

iPhone 画面のいちばん下、接続の情報の下に「開発者にコーヒーを奢る」がある。
消耗型の App内課金で、送っても機能は変わらない。部品は `~/Claude/shared/TipJar/` が正本で、
このアプリでは製品ID `com.zzzjjj080.banmentask.coffee` を渡している。
課金は App Store と通信するので、掲載文やプライバシーポリシーに「通信しない」とは書かない。
